...
def train(config):
    ...
    for epoch in range(config['epochs']):
        ...
        for batch, data in enumerate(train_data_loader):
            ...
            if config['CRF']:
                loss = model(input_ids=inputs.to(config['device']), token_type_ids=segments_tensors.to(config['device']),
    ...
    for key in config_data:
        ...
        if key == 'token_mapping_rule':
            ...
            if config_data[key]:
                ...
                #non-output
                pass
        elif key == 'update_model' or key == 'special_token' or key == 'load_file':
            if os.path.exists(config_data[key]):
...
def main(config_file):
    ...
    config = {'mode':'', 'seed':812, 'model_name':'', 'tagset_size':4, 'update_model':'', 'epochs':1,
              ...
              'load_file':'', 'token_mapping_rule':token_mapping_rule, 'shuffle':True, 'maxlen':60, 'lr':2e-05, 
              'special_token':[], 'CUDA_ENV_NUM':0, 'test_model':'', 'output_file':'', 'CRF': False, }
