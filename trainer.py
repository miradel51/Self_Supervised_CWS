...
class Trainer:
    ...
    def __init__(
        ...
        mrt_data_collator: Optional[DataCollator] = None,
        **kwargs,
    ):
        if args is None:
        ...
        self.mrt_data_collator = mrt_data_collator
        ...
        self.best_metric_value = -1e19 * self.args.metric_type
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        if self.args.n_mrt > 0:
            return DataLoader(
                self.train_dataset,
                batch_size=self.args.train_batch_size,
                sampler=train_sampler,
                collate_fn=self.mrt_data_collator,
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        for epoch in range(epochs_trained, int(np.ceil(num_train_epochs))):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if self.args.n_mrt > 0:
                    tr_loss += self.training_step(model, inputs, risk=risk)
                else:
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], risk=None) -> float:
        """
        ...
        if self.args.n_mrt > 0:
            ...
            __loss = loss_func(logits, labels)
            __loss = torch.mean(__loss[ : , 1 : -1], dim=1)
            prob = F.softmax(__loss * -self.args.alpha_mrt)
            reg_loss = -torch.sum(torch.exp(__loss * -self.args.alpha_mrt))
            loss = torch.sum(prob * risk) + self.args.reg_lambda * reg_loss
        else:
            loss = outputs[0]            
    ...
    def save_best(self):
        metric_value = self.evaluate()[self.args.main_metric]
        metric_value1 = metric_value * self.args.metric_type
        best_metric_value1 = self.best_metric_value * self.args.metric_type
        logger.info("Previous best %s: %f, current %s: %f" % (self.args.main_metric, self.best_metric_value, self.args.main_metric, metric_value))
        if metric_value1 > best_metric_value1:
            self.best_metric_value = metric_value
            # checkpoint_folder = f"{PREFIX_CHECKPOINT_DIR}-best"
            # output_dir = os.path.join(self.args.output_dir, checkpoint_folder)
            output_dir = self.args.output_dir
            logger.info("Now try to save the best checkpoint to %s" % output_dir)
            if os.path.exists(output_dir):
                logger.info("%s already exists. Now we remove it." % output_dir)
                shutil.rmtree(output_dir)
                logger.info("%s removed successfully." % output_dir)
            self.save_model(output_dir)
            logger.info("Checkpoint saving completed")
    ...
    def prediction_loop(
        ...
        for inputs in tqdm(dataloader, desc=description, disable=disable_tqdm):
            ...
            if logits is not None:
                if self.args.lm_format_pred_result:
                    if preds is None:
                        preds = []
                    O = list(torch.argmax(logits, dim=2).cpu().numpy())
                    logits = logits.permute(0, 2, 1)
                    L = list(loss_func(logits, labels).cpu().numpy())
                    for l, o in zip (L, O):
                        oo = self.tokenizer.convert_ids_to_tokens([int(x) for x in o])
                        preds.append((l, oo))
                else:
